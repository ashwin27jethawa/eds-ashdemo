...
function filterData(searchTerms, data) {
  ...
  data.forEach((result) => {
    ...
    // FIX: Use optional chaining (?.) and a fallback empty string ('')
    ...
    // FIX: Safely combine meta contents
    const title = result.title || "";
    const desc = result.description || "";
    const path = result.path ? result.path.split("/").pop() : "";
    const metaContents = `${title} ${desc} ${path}`.toLowerCase();
...
function searchResultsContainer(block) {
  // console.log("searchresult", block);
